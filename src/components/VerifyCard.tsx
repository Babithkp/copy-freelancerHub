...
import { addDocToVerify } from "@/lib/api/fetch";

export default function VerifyCard({params}:any) {
    ...
    if(codeMatch){
      ...
      if(toVerifyDoc){
        ...
        if(docUrl){
          ...
          try{
            const response = await addDocToVerify(params.userid,docInfo)
            if(response){
              ...
              setIsSubmitting(false)
              router.push(`/pro/dashboard/${params.userid}`)
            }
          }catch(e){
            setIsSubmitting(false)
